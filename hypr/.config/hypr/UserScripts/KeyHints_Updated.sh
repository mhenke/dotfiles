...
# Updated Key Hints with custom keybinds
...
GDK_BACKEND=$BACKEND yad \
    --center \
    --title="KooL Quick Cheat Sheet - Updated" \
    --no-buttons \
    --list \
    --column=Key: \
    --column=Description: \
...
" SHIFT E" "Exit/Power Menu" "(Wlogout - logout/restart/shutdown)" \
" SHIFT H" "KooL Hyprland Settings Menu" "(Customization settings)" \
"" "" "" \
"CUSTOM KEYBINDS" "" "" \
" C" "VS Code Editor" "(Visual Studio Code)" \
" SHIFT C" "Zed Editor" "(Zed)" \
" F" "Toggle Fullscreen" "(on/off toggle)" \
" N" "WiFi Manager" "(ronema network manager)" \
"" "" "" \
"BASIC APPS" "" "" \
...
" ALT + Arrow Keys" "Swap window positions" "(swap with adjacent)" \
" CTRL + Arrow Keys" "Move window" "(physically move)" \
" SHIFT + Arrow Keys" "Resize window" "(in 50px increments)" \
...
" SHIFT F" "Fullscreen (whole)" "Full screen mode" \
" CTL F" "Fake Fullscreen" "Fake full screen" \
